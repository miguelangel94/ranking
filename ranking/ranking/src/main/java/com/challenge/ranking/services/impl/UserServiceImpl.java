...
public class UserServiceImpl implements UserService {
    ...
    public List<UserRest> getUsers() throws RankingException {
        return userRepository.findAll().stream().map(user -> userConverter.mapToRest(user)).collect(Collectors.toList());
    }

    @Override
    public UserRest findById(Long id) throws RankingException {
        User user;
        user = userRepository.findById(id).orElseThrow(() -> new NotFoundException("error, user not found"));
